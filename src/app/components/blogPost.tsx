        ...
        <>
            <div className={`p-5 px-10 bg-white m-4 rounded-xl w-[700px] ${ visible ? "visible" : "hidden" }`} onMouseLeave={() => setHovering(false)}  onMouseEnter={() => setHovering(true)} >
                <div className="w-full flex justify-end">
                    <input type="button" onClick={e => hidePost()} value="x" className={`bg-red-500 text-xl border-black/25 hover:border-none border-2 text-black/50 hover:text-white w-8 h-8 font-bold rounded-full absolute self-end mx-[-50px] ${hovering ? "visible" : "hidden"} cursor-pointer transition-all`}></input>
                </div>
